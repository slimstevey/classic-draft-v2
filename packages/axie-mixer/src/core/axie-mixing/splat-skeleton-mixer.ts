...
import { boneComboTypes } from '../common/genes/body-structure'
import { MixedSkeletonData, SampleSkeletonData } from '../common/samples/sample-skeleton-data'
...
export const addCustomBoneRule = (edges: { [key: string]: Array<string> }, sortingRules: { [key: string]: Array<string> }) => {
...
export const mixEntriesFromSample = (
  ...
  const bodyNodes = mapEntries(samples.get(boneCombo[0]) as SampleSkeletonData)
  ...
  for (let i = 1; i < boneComboTypes.length; i++) {
    ...
    const bodyNodes = mapEntries(samples.get(boneCombo[i]) as SampleSkeletonData)
  ...
  function visit(name: string, visited: { [key: string]: boolean }, edges: { [key: string]: Array<string> }, sortedNames: Array<string>) {
    if (!visited[name]) {
      visited[name] = true
...
export const mixSkins = (
  ...
  const getSlotInSampleSkins = (skinName: string, slotName: string, sampleName: string) => {
    const sample = samples.get(sampleName)
...
export const mix = (
  ...
  let bones = mixEntriesFromSample((sample) => sample.bones, boneCombo, samples, sortingRules)
  bones = correctBones(bones)
  const boneNames = bones.map((bone) => bone['name'])

  const slots = mixEntriesFromSample((sample) => sample.slots, boneCombo, samples, sortingRules)
  const slotNames = slots.map((slot) => slot['name'])

  const ik = mixEntriesFromSample((sample) => sample.ik, boneCombo, samples, sortingRules)
