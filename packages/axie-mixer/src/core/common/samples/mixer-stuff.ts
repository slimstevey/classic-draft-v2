...
import { getSampleName, mix } from '../../axie-mixing/splat-skeleton-mixer'
import { ACESSORY_SLOTS, boneComboTypes } from '../genes/body-structure'
import { AnimationHeader, SampleAnimationData } from './sample-animation-data'
import { MixedSkeletonData, SampleSkeletonData } from './sample-skeleton-data'
...
class AxieMierStuff {
  ...
  public generateAsset(adultCombo: Map<string, string>, skipAnim: boolean, debug = false): MixedSkeletonData {
              ...
              for (const drawOrderMap in sampleAnimation.drawOrder) {
                ...
                if (offsets) {
                  for (const offsetMap of offsets) {
                    const slotIndex = slots.findIndex((x) => x.name === offsetMap.slot)
                    const bodySlotIndex = slots.findIndex((x) => x.name === 'body')
