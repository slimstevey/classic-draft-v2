...
import { type AxieBodyStructure, type AxiePartStructure, axiePartTypes, getCharacterClassFromValue } from './body-structure'
...
const bitsFromLeft = (value: BigNumber, totalBits: number): BitsFromLeft => {
  ...
  extractor.peek = (bits) => (extractor.bitsLeft > bits ? value.shrn(extractor.bitsLeft - bits).maskn(bits) : value.maskn(bits))
