...
class AxieGenesStuff {
  ...
  private getBodySample(value: number, skin: number, isMystic: boolean): string {
    ...
    for (let i = 0; i < this.bodySamples.length; i++) {
      ...
      if (bodySample.mysticValue != -1 && bodySample.mysticValue != (isMystic ? 1 : 0)) continue
  ...
  private getPartSample(partType: AxiePartType, bodyStructure: AxieBodyStructure, debug = false): string | null {
    ...
    if (part.skin >= 0 && part.skin < partSample.skins.length) {
      if (partSample.skins[part.skin] && partSample.skins[part.skin].length > 0) {
        return partSample.skins[part.skin] + (part.stage == 1 ? '-lv2' : '')
      }
    }
    return partSample.skins[0] + (part.stage == 1 ? '-lv2' : '')
  }
  ...
  public getAdultCombo(bodyStructure: AxieBodyStructure): Map<string, string> {
    ...
    axieCombo.set('mouth', this.getPartSample(AxiePartType.Mouth, bodyStructure))
  ...
  public getAxieColorsVariant(bodyStructure: AxieBodyStructure): number {
    ...
    for (let i = 0; i < this.axieSkinColors.length; i++) {
      ...
      if (
        (axieSkin.class === CharacterClass.Any || axieSkin.class === bodyStructure.class) &&
