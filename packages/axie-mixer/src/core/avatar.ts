...
import { MixedSkeletonData } from './common/samples/sample-skeleton-data'
...
class BoneMatrix {
  ...
  public static CalculateSetupWorld(boneData: BoneData): BoneMatrix {
    ...
    if (boneData.parent === null) return BoneMatrix.GetInheritedInternal(boneData, new BoneMatrix(null))
    const result = BoneMatrix.CalculateSetupWorld(boneData.parent)
    return BoneMatrix.GetInheritedInternal(boneData, result)
  }

  public static GetInheritedInternal(boneData: BoneData, parentMatrix: BoneMatrix): BoneMatrix {
...
export const exportAvatarLayers = (
  ...
  for (const slotName in skinAttachments) {
    ...
    for (const attachmentName in skinSlotAttachments) {
      ...
      if (attachmentName.startsWith('ear-right-dot') || attachmentName.startsWith('ear-left-dot')) continue
